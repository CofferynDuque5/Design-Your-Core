...
La app móvil tiene una pestaña **Más** con el mismo catálogo que la web (`TOOL_CATALOG` en `@dyc/core`: grupos, textos y cuentas). Tienen pantalla nativa las tandas 1 y 2: **Agenda** (`/agenda`, con `?vista=tareas`), **Pendientes**, **Calendario**, **Horario**, **Enfoque**, **Materias**, **Proyectos**, **Roadmaps**, **Cuadernos** (`/cuadernos` y `/cuadernos/:id`), **Contenido** e **Ideas**. Las demás se muestran como «En la web» y se abren en el navegador (`EXPO_PUBLIC_WEB_URL`).
